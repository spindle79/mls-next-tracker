...
export function EmptyTableRow({
  ...
  return (
    <tr>
      <td colSpan={colSpan} className={center ? 'tracker-table-empty tracker-table-empty--center' : 'tracker-table-empty'}>
        {children}
      </td>
    </tr>
  );
...
export function TrackerState({ variant, children }: { variant: 'loading' | 'error'; children: ReactNode }) {
  return (
    <div className="container">
      <p className={`tracker-state ${variant === 'error' ? 'tracker-state--error' : 'tracker-state--muted'}`}>
        {children}
      </p>
    </div>
  );
