async (page) => {
  const data = await page.evaluate(async () => {
    ...
    for (const c of containers) {
      ...
      if (title && title.getAttribute("data-title").includes("Northern California")) {
        norcal = c;
        break;
      }
    }
    ...
    for (const row of norcal.querySelectorAll(".container-division-row")) {
      ...
      const name =
        mainRow
          .querySelector(".container-team-info p[data-title]")
    ...
    function parseMatches(html) {
      ...
      for (const mr of rows) {
        ...
        const matchId = midDiv
          ? midDiv.textContent.trim().split("\n")[0].replace("MALE", "").trim()
          : "";
        const detailsDiv = mr.querySelector(".col-sm-2");
        let date = "",
          venue = "";
        if (detailsDiv) {
          const dateMatch = detailsDiv.textContent
            .trim()
            .match(/(\d{2}\/\d{2}\/\d{2}\s+\d{1,2}:\d{2}[ap]m)/);
        ...
        const home =
          mr
            .querySelector(".container-first-team p[data-title]")
            ?.getAttribute("data-title")
            ?.trim() || "";
        const away =
          mr
            .querySelector(".container-second-team p[data-title]")
            ?.getAttribute("data-title")
            ?.trim() || "";
